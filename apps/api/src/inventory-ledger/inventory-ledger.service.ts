...
export class InventoryLedgerService {
  ...
  async listLotBalances(
    tenantId: string,
    query: QueryInventoryLotBalancesInput,
  ) {
  ...
  async createEntry(input: CreateInventoryLedgerEntryInput) {
    ...
    if (POSITIVE_ONLY_TYPES.has(input.entryType) && input.quantityDelta < 0) {
      throw new BadRequestException(
        `${input.entryType} requires a positive quantityDelta.`,
      );
    }

    if (NEGATIVE_ONLY_TYPES.has(input.entryType) && input.quantityDelta > 0) {
      throw new BadRequestException(
